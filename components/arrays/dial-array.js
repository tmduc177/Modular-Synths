...
export class DialArray extends BaseComponentArray {
    ...
    drawLeader(options = {}) {
        ...
        var big_dial_constraints = {...this.component_constraints};
    ...
    drawMatrix(options = {}) {
        ...
        var first_dial_params = overwrite_params ? overwrite_params : this.component_constraints;
        ...
        makeRow(first_dial, total_cols, shift_x, stagger_y, this.grid_size, this.components);
        for (var i = 1; i < total_rows; i++) {
            ...
            this.components.push(cloned_first_dial)
            makeRow(cloned_first_dial, total_cols, shift_x, stagger_y, this.grid_size, this.components);
        };
    };

    drawOrbit(options = {}) {
        const { orbit_quantity = getRandomInt(2, 8) } = options;
        var center_constraints = {...this.component_constraints};
        ...
        for (var i = 0; i < orbit_quantity; i++) {
            ...
            orbit_dial.draw();
            this.components.push(orbit_dial)
        };
    };
};
